...
final class VideoUploadController
{
    ...
    public function upload(Request $request): array
    {
        ...
        if (empty($_FILES['video'])) {
            throw new RuntimeException('No video file received. Send the file as multipart/form-data with field name "video".', 422);
        }

        $file = $_FILES['video'];

        if ($file['error'] !== UPLOAD_ERR_OK) {
            throw new RuntimeException($this->uploadErrorMessage($file['error']), 422);
        }

        // ── 2. Validate size ─────────────────────────────────────
        if ($file['size'] > self::MAX_BYTES) {
            $maxMb = self::MAX_BYTES / 1024 / 1024;
            throw new RuntimeException("Video exceeds the {$maxMb} MB limit.", 422);
        }
